...
通过 Linux 系统 D-Bus 直接与 BlueZ 通信的高性能 Home Assistant 蓝牙追踪器（`device_tracker`）。支持纯 UI 图形界面配置，完全不依赖 YAML，内置三层混合在线判定算法！
...
* **三层混合在线判定算法**：
  ...
  2. **Layer 2 (BlueZ 对象树)**：查询 Linux 系统 D-Bus 下的 ObjectManager 活跃 BLE 对象。
  3. **Layer 3 (BlueZ ConnectDevice 探针)**：针对休眠或不主动广播的手机/经典蓝牙，发起底层无感链路探针。
